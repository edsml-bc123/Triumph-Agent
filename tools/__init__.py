...
2. 基础原子工具集 (Layer 2 基础层)：
   - BuiltinTool: read_file, write_file, edit_file, glob, bash
3. 高层领域扩展工具集 (各业务领域高内聚管理，通过 register_plugin 统一装配)：
   ...
   - SkillTool (skills/loader.py): 技能按需渐进式加载工具 (load_skill)
   - DAGTaskTool (orchestration/dag.py): 有向无环图任务依赖调度工具集
   - JobTool (orchestration/job.py): 后台异步长作业托管工具集
"""
