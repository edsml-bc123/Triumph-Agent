...
def _smoke_test():
    import tempfile
    print("启动 security/permission.py 权限钩子自测...")

    with tempfile.TemporaryDirectory() as tmpdir:
        sandbox_dir = Path(tmpdir)
        # 非交互模式用于单测自动断言
        engine = PermissionHook(workdir=sandbox_dir, interactive=False)

        # 1. 验证只读操作放行
        act, _ = engine.evaluate("read_file", {"path": "a.txt"})
        assert act == PermissionAction.AUTO
        print("1. read_file 只读自动放行通过")

        # 2. 验证工作区逃逸一票否决
        act, reason = engine.evaluate("read_file", {"path": "../../etc/passwd"})
        assert act == PermissionAction.DENY
        print(f"2. 路径逃逸拦截通过: {reason}")

        # 3. 验证敏感文件一票否决
        act, reason = engine.evaluate("read_file", {"path": ".env"})
        assert act == PermissionAction.DENY
        print(f"3. 敏感文件拦截通过: {reason}")

        # 4. 验证高危 rm -rf / 一票否决
        act, reason = engine.evaluate("bash", {"command": "rm -rf /"})
        assert act == PermissionAction.DENY
        assert "绝对高危黑名单" in reason
        print(f"4. rm -rf / 黑名单拦截通过: {reason}")

        # 5. 验证 sudo 提权一票否决
        act, reason = engine.evaluate("bash", {"command": "sudo apt-get update"})
        assert act == PermissionAction.DENY
        print(f"5. sudo 提权拦截通过: {reason}")

        # 6. 验证常规删除命令需要人类审核 (ASK)
        act, reason = engine.evaluate("bash", {"command": "rm file.txt"})
        assert act == PermissionAction.ASK
        print(f"6. rm 普通删除转为人工确认通过: {reason}")

        # 7. 验证 PreToolUse 钩子接口返回契约
        # (在非交互模式下，ASK 默认拒绝返回错误字符串，DENY 直接返回错误字符串，AUTO 返回 None)
        err_deny = engine.check("bash", {"command": "rm -rf /"})
        assert err_deny is not None and "Permission Denied" in err_deny

        err_ask = engine.check("bash", {"command": "rm a.txt"})
        assert err_ask is not None and "Operation declined" in err_ask

        err_auto = engine.check("bash", {"command": "ls -la"})
        assert err_auto is None
        print("7. PreToolUse 钩子契约返回值校验通过")

    print("security/permission.py 安全审计自测全部通过！")
