...
def test_tools_file_lifecycle(temp_workspace):
    registry = ToolRegistry(workdir=temp_workspace)

    # 1. 写入文件
    write_res = registry.execute("write_file", {"path": "sub/test.txt", "content": "Line 1\nLine 2\nLine 3"})
    assert "Successfully wrote" in write_res
    assert (temp_workspace / "sub/test.txt").exists()

    # 2. 读取文件
    read_res = registry.execute("read_file", {"path": "sub/test.txt"})
    assert "Line 1\nLine 2\nLine 3" in read_res

    # 读取限制行数
    limit_res = registry.execute("read_file", {"path": "sub/test.txt", "limit": 2})
    assert "Line 1\nLine 2" in limit_res
    assert "more lines" in limit_res

    # 3. 编辑文件
    edit_res = registry.execute("edit_file", {"path": "sub/test.txt", "old_text": "Line 2", "new_text": "Modified"})
    assert "Successfully edited" in edit_res
    new_content = (temp_workspace / "sub/test.txt").read_text(encoding="utf-8")
    assert "Line 1\nModified\nLine 3" == new_content

    # 编辑不存在的内容应返回明确错误
    edit_fail = registry.execute("edit_file", {"path": "sub/test.txt", "old_text": "NonExist", "new_text": "X"})
    assert "Error:" in edit_fail


def test_tools_safe_path_sandbox_escape(temp_workspace):
    registry = ToolRegistry(workdir=temp_workspace)

    # 试图逃逸出沙箱
    escape_res = registry.execute("read_file", {"path": "../../etc/passwd"})
    assert "Error reading file:" in escape_res
    assert "安全违规" in escape_res or "沙箱边界" in escape_res


def test_tools_glob(temp_workspace):
    registry = ToolRegistry(workdir=temp_workspace)
    ...
    glob_res = registry.execute("glob", {"pattern": "*.py"})
    assert "a.py" in glob_res
    assert "b.py" in glob_res
    assert "c.txt" not in glob_res


def test_tools_bash_execution(temp_workspace):
    registry = ToolRegistry(workdir=temp_workspace)

    # 正常命令
    res_echo = registry.execute("bash", {"command": "echo 'Hello Bash'"})
    assert "Hello Bash" in res_echo

    # 包含多余 kwargs 的防御性分发测试
    res_defensive = registry.execute("bash", {"command": "echo 'Safe'", "redundant_arg": 123})
